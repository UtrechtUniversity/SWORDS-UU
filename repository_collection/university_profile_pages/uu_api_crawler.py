...
def get_all_employee_github_links(url):#first try for CV, then for profile text and then for profile links
    time.sleep(1)#avoid ban by adding time between requests
    try:
        API_link = requests.get(f"https://www.uu.nl/medewerkers/RestApi/Public/getEmployeeData?page={url}")
        API_json = API_link.json()
        ...
        if "Links" in API_json["Employee"].keys() and  API_json["Employee"]['Links']:
            employee_link_list =  API_json["Employee"]['Links']
            for link_dict in employee_link_list:
                link = link_dict["Url"]
                if link:
...
faculty_ids = []
for i in range(99):#the distribution of employees over these numbers seems to differ from week to week. Highest found value were employee ids were found was 18
...
employee_github_pd = pd.DataFrame(employee_github,  columns=["uu_user_id", "github_user_id"])
...
employee_github_pd.to_csv(Path("repository_collection", "university_profile_pages","results", "employees_github_usernames.csv"), index = False)
