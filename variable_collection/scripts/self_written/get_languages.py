...
# get languages
variables = []
for counter, (url, languages_url) in enumerate(zip(df_repos["html_url"], df_repos["languages_url"])):
...
df_data = pd.DataFrame(variables, columns=["html_url_repository", "language", "num_chars"])
current_date = datetime.today().strftime('%Y-%m-%d')
df_data.to_csv(Path("variable_collection", "output", "languages_"+current_date+".csv"), index=False)