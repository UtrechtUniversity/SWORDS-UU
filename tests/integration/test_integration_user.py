...
def test_search_topics(service):
    topic_repos = get_complete_query_result(f"topic:utrecht-university","SEARCH_REPOS", service)
    ids_topic_repos = get_users_from_repos(topic_repos, service)
    assert len(ids_topic_repos[0]) == 3 and ids_topic_repos[0][0] == "github.com"


def test_search_users(service):
    search_users = get_complete_query_result("utrecht university","SEARCH_USERS", service)
    ids_search_users = get_users_from_users(search_users, service)
    assert len(ids_search_users[0]) == 3 and ids_search_users[0][0] == "github.com"
...
def test_enrich_new_users(users_merged, service):
    results_github_user_api = get_userdata(users_merged.head(3)["user_id"], service)
...
def test_update_new_users(users_enriched_old, users_merged, service):
    ...
    df_users.loc[~df_users["user_id"].isin(df_users_annotated["user_id"].str.lower()),"new_user"] = True
    ...
    df_users_update = pd.merge(df_users[df_users["new_user"]],
                                ...
                                right_on="user_id",
                                how="left")
    results_github_user_api = get_userdata(df_users_update.head(2)["user_id"], service)
